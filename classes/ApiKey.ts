...
export class ApiKey {
    ...
    create = async (createKeyRequest: ICreateKeyRequest): Promise<object> => {
        try {
            let response;
            this.db = new dbService(this.API_KEY_STORE_DATABASE_ID);
    ...
    logRequest = async (key: IKey) => {
        try {
            this.db = new dbService(this.API_GATEWATE_STORE_DATABASE_ID);
    ...
    verify = async (verifyKeyRequest: IVerifyKeyRequest) => {
        try {
            ...
            await this.db.update(id, params)
                .then((data) => {
                    response = {
    ...
    authorise = async (authoriseKeyRequest: IAuthoriseKeyRequest) => {
        try {
            ...
            await this.db.scan(params).then((data) => {
                key = data.Items[0];
            }).catch((error) => {
                throw new Error(error.message);
            });
